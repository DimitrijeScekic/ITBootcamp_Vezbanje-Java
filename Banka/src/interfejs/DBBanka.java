...
public class DBBanka {
	...
	public void makePaymentSQLInjectionAnomaly(int idrac, String ammount) throws SQLException {
		...
		try (Connection connection = DriverManager.getConnection(connectionString);
			...
			// statement.executeUpdate koristiti samo za NE-SELECT upite (INSERT, UPDATE,
			// CREATE...)
	...
	public void makePayment(int idrac, String ammount) throws SQLException {
		...
		try (Connection connection = DriverManager.getConnection(connectionString);
				PreparedStatement statement = connection
						.prepareStatement("UPDATE Racun SET Stanje=Stanje+? WHERE idrac=?")) {
            // PreparedStatement se koristi samo za UPDATE DELETE i INSERT 
			// Statement samo za SELECT
			// postavljanje parametara - prvi argument predstavlja redni broj (broji se od
			// 1)
	...
	public void makeTransfer(int fromIdRac, int toIdRac, float amount) throws Exception {
		try (Connection connection = DriverManager.getConnection(connectionString);
			...
			// Commit-ujemo prethodne update upite, cime ih smatramo kao jednu celinu za
			// izvrsavanje
			connection.commit();
